...
export async function POST(request: NextRequest) {
    try {
        ...
        // Validate role (default to OFFICER if not specified or invalid)
        const validRoles = ['ANALYST', 'OFFICER', 'COMMANDER', 'ADMIN']
        const userRole = (validRoles.includes(role) ? role : 'OFFICER') as any

        // Map role to clearance level
        const clearanceLevels: Record<string, number> = { ANALYST: 1, OFFICER: 2, COMMANDER: 3, ADMIN: 4 }
